...
func main() {
	...
	var users []User
	DB.Scopes(UsersFromDomain("gmail.com")).Preload("Orders", CardOrders).Find(&users) // Now using scopes within an association
	...
	fmt.Printf("Orders from a user (%s): \n", users[0].Email) // great combination of scope with association in action for filtering
